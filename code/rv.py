...
class Variable(set, metaclass=utils.CopiedType):
    ...
    def copy(self, **newparams) -> 'Variable':
        d = {k:v for k,v in self.__dict__.items() if k in Variable.PARAMS}
        d.update(**newparams)
        duplicate = Variable(self.ordered, **d)
    ...
    def __eq__(self, other):
        seteq = set.__eq__(self,other)
        if hasattr(self, "name") and hasattr(other, "name"):
            return seteq and (self.name == other.name)
        return seteq
